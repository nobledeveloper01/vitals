...
void main() {
  test('strings of every UTF-8 width round-trip', () {
    for (final s in ['nurse-a', 'Adé', 'Ọlá', '孙', '🩺 ward', '']) {
      final f = Fact(
          ...
          patient: patientId(1),
          kind: FactKind.note,
          stamp: Stamp(wallMillis: -5, counter: 0, device: s),
          author: s,
          payload: const [],
          supersedes: null);
      ...
      expect(back.all.single.stamp.wallMillis, -5,
          reason: 'a negative wall time survives');
    }
  });
  ...
  test('a supersedes id round-trips and a missing one is a zero byte', () {
    ...
    final s = fact(2,
        device: 'a', wall: 11, kind: FactKind.supersession, supersedes: a.id);
  ...
  test(
      'truncation, a wrong version, trailing bytes and an overlong id are refused',
      () {
    final bytes = Canonical.bytesOf(
        Record.of(patientId(1), [fact(1, device: 'a', wall: 1)]));
    for (var n = 0; n < bytes.length; n += 7) {
      expect(() => Canonical.recordFrom(bytes.sublist(0, n)),
          throwsA(isA<CanonicalError>()),
          reason: 'cut at $n');
    }
    expect(() => Canonical.recordFrom([2, ...bytes.skip(1)]),
        throwsA(isA<CanonicalError>()));
    expect(() => Canonical.recordFrom([...bytes, 0]),
        throwsA(isA<CanonicalError>()));
    final long = Fact(
        id: List.filled(300, 1),
        patient: patientId(1),
        kind: FactKind.note,
        stamp: const Stamp(wallMillis: 1, counter: 0, device: 'a'),
        author: 'a',
        payload: const [],
        supersedes: null);
    expect(() => Canonical.bytesOf(Record.of(patientId(1), [long])),
        throwsA(isA<CanonicalError>()));
    expect(const CanonicalError('x').toString(), 'CanonicalError: x');
  });

  test('the fixture: these bytes, for ever', () {
    final r = Record.of(patientId(1), [
      fact(1, device: 'a', wall: 1_700_000_000_000),
      fact(2,
          device: 'b',
          wall: 1_700_000_000_500,
    ...
    expect(bytes.sublist(wallAt, wallAt + 8),
        [0, 0, 1, 0x8b, 0xcf, 0xe5, 0x68, 0]);
  });
}
