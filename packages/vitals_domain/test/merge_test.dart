    ...
    for (var s = 0; s < steps; s++) {
      ...
      switch (gen.next(4)) {
        case 0 || 1: // record a fact
          wall += gen.next(5000);
          final f = fact(seed * 1000 + s, device: d, wall: wall, kind: FactKind.values[gen.next(6)]);
          everRecorded.add(f);
        ...
        case 2: // correct a fact this device holds
          ...
          wall += gen.next(5000);
          final sup = fact(seed * 1000 + s, device: d, wall: wall, kind: FactKind.supersession, supersedes: target.id);
...
void main() {
  ...
  test('1. no fact is ever lost: after settling, every device holds everything ever recorded', () {
    for (var seed = 1; seed <= seeds; seed++) {
      ...
      for (final r in w.held.values) {
        ...
        for (final f in w.everRecorded) {
          expect(r.contains(f), isTrue, reason: 'seed $seed lost ${f.key.substring(0, 8)}');
        }
      }
    }
  });
  ...
  test('2. merge is commutative, associative and idempotent', () {
    for (var seed = 1; seed <= seeds; seed++) {
      ...
      expect(Canonical.bytesOf(Merge.union(a, b)), Canonical.bytesOf(Merge.union(b, a)), reason: 'commutative, seed $seed');
      expect(Canonical.bytesOf(Merge.union(Merge.union(a, b), c)), Canonical.bytesOf(Merge.union(a, Merge.union(b, c))), reason: 'associative, seed $seed');
      expect(Canonical.bytesOf(Merge.union(a, a)), Canonical.bytesOf(a), reason: 'idempotent, seed $seed');
    }
  });

  test('3. a correction never hides the original: both survive and the history reaches back', () {
    var seen = 0;
    for (var seed = 1; seed <= seeds; seed++) {
      final w = World(seed)..run()..settle();
      ...
      for (final s in supersessions) {
        final original = r.all.firstWhere((f) => f.key == Record.of(f.patient, [f]).all.first.key && _same(f.id, s.supersedes!));
        expect(r.contains(original), isTrue, reason: 'seed $seed');
        expect(r.current.contains(original), isFalse, reason: 'a superseded fact is not shown as current, seed $seed');
        expect(r.history(original).map((h) => h.key), contains(s.key), reason: 'seed $seed');
      }
    }
    // A generator that never corrects makes this test vacuous; it did once.
    expect(seen, greaterThan(100), reason: 'the worlds must contain corrections for this to test anything');
  });
  ...
  test('4. two devices that exchanged everything hold identical bytes', () {
    for (var seed = 1; seed <= seeds; seed++) {
      ...
      expect(Canonical.bytesOf(Canonical.recordFrom(bytes.first)), bytes.first, reason: 'seed $seed');
    }
  });

  test('5. what one device accepted is accepted everywhere: any subset merges into any other without loss', () {
    for (var seed = 1; seed <= seeds; seed++) {
      final w = World(seed)..run();
