...
final class DueChild {
  const DueChild({required this.patient, required this.registration, required this.due, required this.mostOverdueDays});
...
abstract final class Whiteboard {
  static List<DueChild> today(Iterable<Record> records, {required int todayDays}) {
    final out = <DueChild>[];
    for (final r in records) {
      final reg = Registration.of(r.current);
      if (reg == null) continue;
      final card = Card.of(bornDays: reg.bornDays, given: Given.of(r.current), todayDays: todayDays);
      ...
      out.add(DueChild(patient: r.patient, registration: reg, due: due, mostOverdueDays: behind));
    }
    out.sort((a, b) {
      final c = b.mostOverdueDays.compareTo(a.mostOverdueDays);
      return c != 0 ? c : a.registration.fullName.compareTo(b.registration.fullName);
    });
    return out;
  }

  /// Defaulters: children with at least one dose overdue.
  static List<DueChild> defaulters(Iterable<Record> records, {required int todayDays}) =>
      today(records, todayDays: todayDays).where((c) => c.mostOverdueDays > 0).toList();
}
